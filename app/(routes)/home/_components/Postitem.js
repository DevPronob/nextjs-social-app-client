...
function Postitem({post,updatedPostList}) {
    const { userDetail, setUserDetail } = useContext(UserDetailContext);
    const { user } = useUser();
    const [userInputComment, setUserInputComment] = useState();

    const chceckIsUserLike =(postLikes) =>{
        if (Array.isArray(postLikes) && postLikes.length > 0) {
            // Find the current user's id in the postLikes array
            return postLikes.find(item => item === userDetail?._id) !== undefined;
        }
        return false; 
    }
    ...
    const addComment =(post) =>{
        ...
        GlobalApi.createComment(data).then(resp => {
            if (resp) {
                toast.success("Your Comment added succesasfully")
                updatedPostList();
            }
        })
        setUserInputComment('')
           ...
           <div className='m-4'>
            {
                post?.comments.map(post =>{
                    return  <CommentList commentList={post} 
                    userDetail={userDetail}
